...
function checkQualityGateStatus(params) {
  ...
  // resolve environment variables in the projectKey
  const resolvedProjectKey = resolveProjectKey(params["sonar.projectKey"]);
  console.log("Resolved sonar.projectKey=" + resolvedProjectKey);
...
/**
 * This function resolves the sonar.projectKey by replacing environment
 * variable expressions by their value
 * @param {*} projectKey The sonar projectKey expression to resolve
 */
function resolveProjectKey(projectKey) {
  const envVarRegex = /\$\{((?!\}).)*\}/g;
  var evaluatedprojectKey = projectKey.replace(envVarRegex, envVarReplace);
  return evaluatedprojectKey;
}

/**
 * This function is called as a parameter of the replace function and replaces
 * environment variable expressions by their value
 * @param  {...any} params These params are provided by the replace function
 */
function envVarReplace(...params) {
  const matchedEnvVar = params[0];
  debug("Matched environment variable=" + matchedEnvVar);
  const envVarName = matchedEnvVar.replace("${env.", "").replace("}", "");
  const evaluatedEnvVar = eval("process.env." + envVarName);
  debug(envVarName + " environment variable value=" + evaluatedEnvVar);
  return evaluatedEnvVar;
}
